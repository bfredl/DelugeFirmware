...
DexedParam dexedParam{l10n::String::STRING_FOR_DEXED_PARAM};

void DexedParam::beginSession(MenuItem* navigatedBackwardFrom) {
	readValueAgain();
}

void DexedParam::readValueAgain() {
	patch = soundEditor.currentSource->ensureDx7Patch();
	displayValue = patch->currentPatch[param];
...
void DexedParam::selectEncoderAction(int32_t offset) {
	int value = patch->currentPatch[param];
...
void DexedParam::horizontalEncoderAction(int32_t offset) {
	if (Buttons::isShiftButtonPressed()) {
		if (param > 6*21+8) return; // TODO: remember last OP param?
...
bool DexedParam::potentialShortcutPadAction(int32_t x, int32_t y, bool on) {
	int found_param = -1;
	if (y > 1 && x <= 13) {
		int op = 8 - y - 1; // op 0-5
		int idx;
...
[[nodiscard]] std::string_view DexedParam::getTitle() const {
...
static void renderAlgorithm(uint8_t *patch) {
	...
	OLED::drawString(buffer, 113, 7, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX,
	                 kTextSizeYUpdated);
...
void DexedParam::drawPixelsForOled() {
	const int y0 = 20;
	char buffer[12];
	...
	if (param < (6 * 21 + 8) && idx < 8) {
		renderEnvelope(patch->currentPatch, op, idx); // op== 6 for pitch envelope
	}
	else if (param < 6 * 21 && idx < 13) {
		renderScaling(patch->currentPatch, op, idx);
	}
	else if (param < 6 * 21 && idx < 16) {
		renderSensParams(patch->currentPatch, op, idx);
	} else if (param < 6 * 21 && idx < 21) {
		renderTuning(patch->currentPatch, op, idx);
	}
	else if (param == 134) {
		renderAlgorithm(patch->currentPatch);
	}
	else if (param >= 137 && param < 144) {
		renderLFO(patch->currentPatch, param);
	} else {
