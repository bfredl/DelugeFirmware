...
void EmulatedDisplay::writeCurrentValue() {
	...
	if (runtimeFeatureSettings.get(RuntimeFeatureSettingType::EmulatedDisplay) == RuntimeFeatureStateEmulatedDisplay::Hardware) {
		if (is_swapped) {
			hid::display::swapDisplayType();
		}
	}
	else if (runtimeFeatureSettings.get(RuntimeFeatureSettingType::EmulatedDisplay) == RuntimeFeatureStateEmulatedDisplay::OnBoot) {
		if (!is_swapped) {
			hid::display::swapDisplayType();
		}
	}
