...
#include "dexed_operator_params.h"
...
DexedOperatorParams dexedOperatorParams{l10n::String::STRING_FOR_DEXED_GLOBAL_PARAMS,
                                        l10n::String::STRING_FOR_DEXED_OPERATOR_PARAMS};

void DexedOperatorParams::beginSession(MenuItem* navigatedBackwardFrom) {
	readValueAgain();
}

void DexedOperatorParams::readValueAgain() {
	if (display->haveOLED()) {
		renderUIsForOled();
	}
	else {
...
struct {
    ...
    {"pitch", 18},       {"level", 16},        {"envelope", 0}, {"velocity sense", 15},
    {"note scaling", 8}, {"rate scaling", 13}, {"ampmod", 14},
};

constexpr int numValues = sizeof(items) / sizeof(items[0]);

void DexedOperatorParams::drawPixelsForOled() {
...
void DexedOperatorParams::drawValue() {
	display->setScrollingText(items[currentValue].name);
}

void DexedOperatorParams::selectEncoderAction(int32_t offset) {
...
MenuItem* DexedOperatorParams::selectButtonPress() {
	int index = items[currentValue].index;
	dexedParam.param = op * 21 + index;
	return &dexedParam;
}
