...
const char* desc_op_long[] = {"env rate1",   "env rate2",  "env rate3",  "env rate4",     "env level1",  "env level2",
                              ...
                              "coarse",      "fine",       "detune"};
const char* desc_op_short[] = {"rat1",   "rat2",  "rat3",  "rat4",   "lvl1",  "lvl2",
                              "lvl3",  "lvl4", "brkp", "le depth",  "ri depth", "le curve",
                              "ri curve", "rate scale", "ampmod",     "velo sens", "levl",       "mode",
                              "coar",      "fine",       "detune"};
...
const char* desc_global_short[]{"piR1", "piR2",     "piR3",  "piR4",  "pil1",
                               "pil2", "pil3",     "pil4",  "algo",  "fdbk",
                               "oscSync",  "LFO rate",     " LFO delay", "LFO pitch", "LFO amp",
                               "LFO sync", "LFO wave", "pitch sens"};
...
void DxParam::drawValue() {
	if (display->haveOLED()) {
		...
		// TODO: a few more strings, like curves
		if (text) {
