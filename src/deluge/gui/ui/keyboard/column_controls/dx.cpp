...
static Dx7Patch *getCurrentPatch() {
	auto *inst = getCurrentInstrument();
	if (inst->type == OutputType::SYNTH) {
		auto *sound = (SoundInstrument*)inst;
		if (sound->sources[0].oscType == OscType::DEXED) {
			return sound->sources[0].dx7Patch;
		}
	}
	return nullptr;
}
...
void DXColumn::renderColumn(RGB image[][kDisplayWidth + kSideBarWidth], int32_t column) {
	Dx7Patch *patch = getCurrentPatch();
	if (!patch) {
		return;
	}
	int algid = patch->currentPatch[134];
