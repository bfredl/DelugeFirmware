...
void Voice::noteOff(ModelStackWithVoice* modelStack, bool allowReleaseStage) {
	...
	if (sound->synthMode != SynthMode::FM) {
		for (int32_t s = 0; s < kNumSources; s++) {
			...
			} else if (sound->sources[s].oscType == OscType::DEXED) {
...
dontUseCache: {}
		...
		// Or echoing input
			...
			Dx7VoiceCtrl ctrl{};
			ctrl.ratemod = paramFinalValues[params::LOCAL_OSC_A_PHASE_WIDTH + s] >> 16;
			ctrl.ampmod = paramFinalValues[params::LOCAL_CARRIER_0_FEEDBACK + s] >> 16;
			...
			if (stereoUnison) {
				for (int i = 0; i < numSamples; i++) {
					sourceAmplitudeNow += amplitudeIncrement;
					int amplified = multiply_32x32_rshift32(uniBuf[i], sourceAmplitudeNow) << 8;
