...
void Dx7Patch::updateEngineMode() {
	core = Dexed::engineModern;
#ifdef USE_DEXED_MK1
	if (engineMode == 2) {
	core = Dexed::engineMkI;
	} else if (engineMode == 0) {
		int algo = currentPatch[134];
		int feedback = currentPatch[135];
...
void Dx7Patch::setEngineMode(int mode, bool neon) {
	// TODO: neon should be the only option for modern in release :P
	Dexed::engineModern->neon = neon;
...
void Dx7Patch::computeLfo(int n) {
	int rate = currentPatch[137]; // 0..99
	int sr = rate == 0 ? 1 : (165 * rate) >> 6;
	sr *= sr < 160 ? 11 : (11 + ((sr - 160) >> 4));
	lfo_delta = Dexed::lfo_unit * sr;

	lfo_phase += n * lfo_delta;
	int waveform_ = currentPatch[142];
	if (waveform_ == 5) {
		// TODO: s&h
...
void Dx7Note::init(Dx7Patch& newp, int midinote, int logfreq, int velocity) {
	...
	if (a == 99) {
		...
		a = (16 + (a & 15)) << (1 + (a >> 4));
		delayinc_ = Dexed::lfo_unit * a;
		a &= 0xff80;
		a = max(0x80, a);
		delayinc2_ = Dexed::lfo_unit * a;
	}

	delaystate_ = 0;
}

int32_t Dx7Note::getdelay(int n) {
...
void Dx7Note::compute(int32_t* buf, int n, int base_pitch, const Dx7Patch* ctrls, const Dx7VoiceCtrl* voice_ctrls) {
	// assert(n <= DX_MAX_N);
	// LFO delay
...
void Dx7Note::updateBasePitches(int logFreq_for_detune) {
	for (int op = 0; op < 6; op++) {
		int off = op * 21;
		int mode = patch[off + 17];
		int coarse = patch[off + 18];
