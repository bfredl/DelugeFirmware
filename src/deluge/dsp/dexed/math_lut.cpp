...
int32_t Freqlut::lookup(int32_t logfreq) {
	...
	int32_t y0 = lut_data->freq_lut[ix];
	int32_t y1 = lut_data->freq_lut[ix + 1];
...
void dexed_init_lut_data() {
	lut_data = (DexedLutData*)GeneralMemoryAllocator::get().allocMaxSpeed(sizeof(*lut_data));
	exp2_init(lut_data->exp2tab);
	tanh_init(lut_data->tanhtab);
	sin_init(lut_data->sintab);
	freq_lut_init(lut_data->freq_lut, 44100);
}
