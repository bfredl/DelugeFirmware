...
DxVoice* Dexed::solicitDxVoice() {
	if (firstUnassignedDxVoice) {
		...
		void* memory = GeneralMemoryAllocator::get().alloc(sizeof(DxVoice), NULL, false, true);
...
Dx7Patch *Dexed::newPatch(void) {
	void* memory = GeneralMemoryAllocator::get().alloc(sizeof(Dx7Patch), NULL, false, true);
