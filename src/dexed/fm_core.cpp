...
void FmCore::render(int32_t *output, int n, FmOpParams *params, int algorithm, int32_t *fb_buf, int32_t feedback_shift) {
        ...
        if (gain1 >= kLevelThresh || gain2 >= kLevelThresh) {
            ...
            if (inbus == 0 || !has_contents[inbus]) {
                ...
                if ((flags & 0xc0) == 0xc0 && feedback_shift < 16) {
                    ...
                    FmOpKernel::compute_pure(outptr, simd_n, param.phase, param.freq,
                                             gain1, gain2, dgain, add, neon);
                }
            } else {
                // cout << op << " normal " << inbus << outbus << " " << param.freq << add << endl;
                FmOpKernel::compute(outptr, simd_n, buf_[inbus - 1].get(),
