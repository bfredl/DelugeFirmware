...
void Dx7UI::openFile(const char *path) {
  ...
  uint8_t* buffer = (uint8_t*)GeneralMemoryAllocator::get().alloc(readsize, NULL, false, true);
  if (!buffer) {
...
void Dx7UI::focusRegained() {
	...
	if (dexedEditedSource) {
		patch = Dexed::ensurePatch(dexedEditedSource);
	} else {
		patch = NULL;
	}
...
ActionResult Dx7UI::padAction(int32_t x, int32_t y, int32_t on) {
  ...
  if (x >= kDisplayWidth) return ActionResult::DEALT_WITH;
		...
		if (result != ActionResult::NOT_DEALT_WITH) return result;
...
ActionResult Dx7UI::potentialShortcutPadAction(int x, int y, int on) {
...
ActionResult Dx7UI::horizontalEncoderAction(int32_t offset) {
	if (getRootUI() == &keyboardScreen) {
		return getRootUI()->horizontalEncoderAction(offset);
	}
}

ActionResult Dx7UI::verticalEncoderAction(int32_t offset, bool inCardRoutine) {
	if (getRootUI() == &keyboardScreen) {
...
bool Dx7UI::renderSidebar(uint32_t whichRows, uint8_t image[][kDisplayWidth + kSideBarWidth][3],
...
bool Dx7UI::renderMainPads(uint32_t whichRows, uint8_t image[][kDisplayWidth + kSideBarWidth][3],
...
void Dx7UI::renderOLED(uint8_t image[][OLED_MAIN_WIDTH_PIXELS]) {
	...
	if (patch && state == kStateEditing && param >= 0) {
		...
		OLED::drawString(buffer, 0, 5, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		...
		if (param < 6*21) {
			...
			OLED::drawString(buffer, 4*kTextSpacingX, 5, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);

			OLED::drawString(desc_op_long[idx], 8*kTextSpacingX, 5, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		} else if (param < 6*21+18) {
			OLED::drawString(desc_global_long[param-6*21], 4*kTextSpacingX, 5, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		}
		...
		OLED::drawString(buffer, 0, ybel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		...
		OLED::drawString(extra, 4*kTextSpacingX, ybel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		...
		OLED::drawString("--- random detune", 0, 5, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
		int ybel = 5+kTextSizeYUpdated+2;
		...
		OLED::drawString(progName, 0, 5+kTextSizeYUpdated+2, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
	} else {
...
void show(const char *text, int r, int c, bool inv = false) {
  ...
  OLED::drawString(text, xpos, ybel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
  if (inv) {
...
void Dx7UI::renderScaling(uint8_t image[][OLED_MAIN_WIDTH_PIXELS], int op, int param) {
  ...
  OLED::drawString(buffer, xpos, ybelmid, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, kTextSpacingX, kTextSizeYUpdated);
  if (8 == param) {
    OLED::invertArea(xpos-1, kTextSpacingX*2+1, ybelmid-1, ybelmid + kTextSizeYUpdated, OLED::oledMainImage);
  }
...
ActionResult Dx7UI::timerCallback() {
	if (timerRedraw) {
		renderUI();
		timerRedraw = false;
	}
...
ActionResult Dx7UI::buttonAction(hid::Button b, bool on, bool inCardRoutine) {
...
int32_t Dx7SyxBrowser::getCurrentFilePath(String* path) {
	int error;
